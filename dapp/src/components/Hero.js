...
const MintConfirmation = lazy(() => import('./MintConfirmation'));
            ...
            <div className='flex flex-col justify-between m-auto h-5/6 w-10/12 pt-10'>
                ...
                <div className='m-auto'>
                    ...
                    <div className='flex flex-col lg:flex-row w-10/12 md:w-3/5 lg:1/2 justify-between mx-auto z-10 pb-0 lg:pb-10'>
                        ...
                        <div 
                        onClick={handleClick}
                        className='whitespace-nowrap hover:cursor-pointer ease-in ease-out duration-300 m-auto mx-auto lg:mx-2 mt-5 lg:mt-0 lg:m-none w-3/5 lg:w-1/2 md:mt-5 bg-cover text-center bg-lime-600 hover:bg-lime-500 text-center rounded-full py-2 lg:py-5' src='/WASiteAssets/DiscordButton.webp' alt='discord button' >
                            { 
                            mintPending ? <Suspense fallback={<div>Loading...</div>}><MintConfirmation error={error} txHash={txHash} reciept={reciept} /></Suspense> :
                            <div>
                            {props.walletAddress.length > 0 ? mintText() : 'Connect Wallet'}
                            </div>
                            }
                        </div>
                        <div onClick={props.openModal} className='whitespace-nowrap hover:bg-blue-600 ease-in ease-out duration-300 m-auto  mx-auto lg:mx-2 mt-5 lg:mt-0 lg:m-none mb-10 lg:mb-0 w-3/5 lg:w-1/2 md:mt-5 bg-cover text-center bg-zinc-600 text-center rounded-full py-2 lg:py-5 hover:cursor-pointer' src='/WASiteAssets/DiscordButton.webp' alt='discord button' >Piece Unique Studio</div>
                    </div>
                </div>
            </div>
        </div>
